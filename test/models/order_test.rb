...
class OrderTest < ActiveSupport::TestCase
  test "generates a random number on create" do
    ...
    #también puede ser: assert not order.number.nil?
  end
  ...
  test "number must be unique" do
    ...
    assert !dup_order.valid?
  end

  test "ads products as order_items" do
    user = users(:one)
    order = Order.create(user_id: user.id)
  ...
  test "products with zero stock can't ne added to cart" do
    user = users(:one)
    order = Order.create(user_id: user.id)
